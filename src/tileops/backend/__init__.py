...
entry, in that order, with an ``optional: true`` input the call did not pass arriving as
``None``, then the construction params under ``signature.params`` names. The kernel it returns
is called with those tensors in the same order, and with a caller-supplied output buffer such
as ``out`` by keyword. Before it runs, every tensor is on one device, every input the call
does not write is contiguous, and the call meets the input dtypes and shape rules the manifest
states; an output buffer meets its output's.
Choosing among its own kernels, constructing, caching and compiling all happen inside it, so
