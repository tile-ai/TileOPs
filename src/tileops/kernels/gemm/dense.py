...
class GemvKernel(Kernel):
    ...
    def default_config(self) -> dict:
        ...
        if sm_version in {90}:
            ...
            # shape policy in ``heuristics.gemv_config``; ``tune=True``
...
class SmallBatchGemmKernel(Kernel):
    ...
    dispatch band live in ``call_spec.small_batch_region``.
    ...
    :func:`~tileops.kernels.gemm.call_spec.small_batch_region`. Everything above
    routes to ``GemmKernel``, whose small-m band picks split-K / simple
    configs (``heuristics._tiny_m_config``).
