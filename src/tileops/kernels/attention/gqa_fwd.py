...
class GQAPrefillWithKVCacheFwdKernel(Kernel):
    ...
    def forward(self, q: torch.Tensor, k_new: torch.Tensor, v_new: torch.Tensor,
        ...
        output, _ = _gqa_prefill_with_kv_cache_fwd_wrapped_kernel(
            self.batch, self.heads, self.heads_kv, self.seq_len_new, self.seqlen_kv, self.dim,
...
class GQAPrefillWithKVCacheRopeFwdKernel(Kernel):
    ...
    def __init__(self,
        ...
        self.init_config(config, tune)
        self._append = GQAPrefillWithKVCacheRopeAppendKernel(
            batch=self.batch,
            heads_kv=self.heads_kv,
            seq_len_new=self.seq_len_new,
    ...
    def forward(self, q: torch.Tensor, k_new: torch.Tensor, v_new: torch.Tensor,
        ...
        if cos_table is None or sin_table is None:
            raise ValueError("GQAPrefillWithKVCacheRopeFwdKernel requires the rotary tables")
        self._append(k_new, v_new, k_cache, v_cache, cache_seqlens, cos_table, sin_table)
        output, _ = _gqa_prefill_with_kv_cache_rope_fwd_wrapped_kernel(
            self.batch, self.heads, self.heads_kv, self.seq_len_new, self.seqlen_kv, self.dim,
            self.max_position, self.rotary_dim, self.is_causal, self.sm_scale, self.softcap,
