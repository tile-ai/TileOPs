...
template <typename OutT>
__device__ __forceinline__ void fp8_fa3_o_smem_store_global_cute_64x128_tail(
    OutT* o_smem, OutT* output, int output_row_stride, int valid_rows) {
  using namespace cute;
  using StoreConfig = FP8Fa3OutputStore64x128<OutT>;
...
#pragma unroll
  for (int linear = tid; linear < 64 * 128; linear += 128) {
    ...
    if (row < valid_rows) {
      output[row * output_row_stride + col] = sO(row, col);
    }
  }
}
}  // namespace tl
