            ...
            with T.Kernel(T.ceildiv(seq_len_q, block_m), heads, batch, threads=threads) as (
                ...
                for i, d in T.Parallel(block_m, dim):
                    if bx * block_m + i < seq_len_q:
                        output[bz, bx * block_m + i, by, d] = T.if_then_else(
                            logsum[i] > 0,
...
@torch.library.custom_op("tileops::gqa_fwd_fp8_general_wrapped_kernel", mutates_args=())
...
def _gqa_fwd_fp8_bn224_tma_v_kernel(
    ...
    def online_softmax_with_causal_partial_sum(
        ...
        for i, j in T.Parallel(half_m, 224):
            acc_s[i, j] = T.exp2(
                acc_s[i, j] * score_scale_softmax - scores_max[i] * scale
            )
        T.call_extern(
            "handle",
            "tl::fp8_partial_row_sum_raw_acc_64x224",
                    ...
                    for tile_b, tile_hkv, tile_m, tile_g in T.Persistent(
                        ...
                        for n_idx in T.Pipelined(loop_range, num_stages=0):
                            ...
                            if (
                                is_causal
                                and (n_idx + 1) * 224
                                > causal_offset + row_base + block_m
                            ):
                                for i, j in T.Parallel(half_m, 224):
...
@torch.library.custom_op("tileops::gqa_dense_fwd_fp8_bn224_wrapped_kernel", mutates_args=())
def _gqa_dense_fwd_fp8_bn224_wrapped_kernel(
    batch: int,
    heads: int,
    heads_kv: int,
    seq_len_q: int,
...
@_gqa_dense_fwd_fp8_bn224_wrapped_kernel.register_fake
def _(
    batch: int,
    heads: int,
    heads_kv: int,
...
class GQADenseFP8Kernel(Kernel):
    """Native-FP8 Dense GQA with a BN224 fast schedule and a general schedule."""
    ...
    def _uses_bn224_fast_schedule(self) -> bool:
        return (
            ...
            and (self.is_causal or self.seq_len_q == self.seq_len_kv)
        )
    ...
    def forward(
        ...
        if self._uses_bn224_fast_schedule():
            return _gqa_dense_fwd_fp8_bn224_wrapped_kernel(
                self.batch,
                self.heads,
                self.heads_kv,
                self.seq_len_q,
        ...
        if self.fuse_rope:
            if rope_cos is None or rope_sin is None:
                raise ValueError("fused RoPE requires cosine and sine tables")
        else:
            rope_cos = rope_sin = q.reshape(-1)[:1].reshape(1, 1)
        return _gqa_fwd_fp8_general_wrapped_kernel(
            self.batch,
            self.heads,
            self.heads_kv,
            self.seq_len_q,
