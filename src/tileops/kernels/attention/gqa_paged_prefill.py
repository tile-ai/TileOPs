    ...
    def build(block_m: int, block_n: int, num_stages: int, threads: int) -> Callable:
        ...
        def quantize_cache(value, scale_value):
            if fp8_cache and not native_fp8:
                return T.clamp(T.Cast("float32", value) / scale_value, -448.0, 448.0)
            return value
        ...
        def main(
            ...
            with T.Kernel(T.ceildiv(max_seqlen_q, block_m), heads, batch, threads=threads) as (
                    ...
                    if bx * block_m + block_m <= q_len:
                        if append_start >> T.int32(page_size_log2) == (append_end - 1) >> T.int32(
                            ...
                            for i, d in T.Parallel(block_m, dim):
                                ...
                                physical_pos = (
                                    block_table[bz, split_page_idx] * page_size + split_page_offset
                                )
                ...
                if is_causal:
                    k_end = T.ceildiv(T.min(total_len, old_len + (bx + 1) * block_m), block_n)
                ...
                if window_size_left >= 0:
                    k_start = T.max(0, old_len + bx * block_m - window_size_left) // block_n
                else:
                    k_start = 0
                ...
                for k_offset in T.Pipelined(loop_range, num_stages=num_stages):
                    ...
                    if tile_end <= old_len:
                        if page_size % block_n == 0:
                            ...
                            for p in range(block_n // page_size):
                                physical_start = block_table[bz, tile_page_start + p] * page_size
                        ...
                        for j, d in T.Parallel(block_n, dim):
                            ...
                            if fuse_rope:
                                ...
                                paired_value = k_new[q_start + new_pos, cur_kv_head, paired_d]
                                k_shared[j, d] = rotate(
                                    value,
                                    paired_value,
                                    logical_pos,
                        ...
                        for j, d in T.Parallel(block_n, dim):
                            ...
                            physical_pos = (
                                block_table[bz, load_page_idx] * page_size + load_page_offset
                            )
                            if kv_pos < old_len:
                                k_shared[j, d] = load_cache(
                                    k_pages[physical_pos, cur_kv_head, d], local_k_scale
                    ...
                    if native_fp8:
                        ...
                        if use_softcap:
                            ...
                            initialize_mask(acc_s, k_idx, bx, q_len, total_len, old_len)
                        ...
                        if use_softcap:
                            apply_softcap(acc_s)
                            apply_transformed_mask(acc_s, k_idx, bx, q_len, total_len, old_len)
                ...
                for i, d in T.Parallel(block_m, dim):
                    if bx * block_m + i < q_len:
                        output[q_start + bx * block_m + i, by, d] = (
                            acc_o[i, d] * inv_logsum[i] * (local_v_scale if native_fp8 else 1.0)
                        )
...
def _gqa_prefill_paged_with_kv_cache_rope_append_kernel(
    ...
    if rope_layout not in ("neox", "interleaved"):
        raise ValueError("rope_layout must be 'neox' or 'interleaved'")
    half = rotary_dim // 2
    quantized_cache = cache_dtype == "float8_e4m3fn" and input_dtype != cache_dtype
    page_size_log2 = page_size.bit_length() - 1
    ...
    def _gqa_prefill_paged_with_kv_cache_rope_append_func(block_m: int, threads: int) -> Callable:
        ...
        _, paired_dim, rotate = make_prefill_rope_policy(True, rotary_dim, rope_layout)

        @T.macro
        def store_value(value, scale_value):
            if quantized_cache:
                return T.clamp(T.Cast("float32", value) / scale_value, -448.0, 448.0)
            return value
                ...
                if bx * block_m + block_m <= q_len:
                    if append_start >> T.int32(page_size_log2) == (append_end - 1) >> T.int32(
                        ...
                        for i, d in T.Parallel(block_m, dim):
                            ...
                            k_pages[physical_start + i, by, d] = store_value(
                                rotate(val, paired_val, logical_pos, d, cos_table, sin_table),
                                k_scale[bz, by],
                            )
                            v_pages[physical_start + i, by, d] = store_value(
                                v_new[q_start + new_pos, by, d], v_scale[bz, by]
                            )
                    else:
                        for i, d in T.Parallel(block_m, dim):
                            ...
                            physical_pos = (
                                block_table[bz, append_page_idx] * page_size + append_page_offset
                            )
                            paired_d = paired_dim(d)
                            val = k_new[q_start + new_pos, by, d]
                            paired_val = k_new[q_start + new_pos, by, paired_d]
                            k_pages[physical_pos, by, d] = store_value(
                                rotate(val, paired_val, logical_pos, d, cos_table, sin_table),
                                k_scale[bz, by],
                            )
                            v_pages[physical_pos, by, d] = store_value(
                                v_new[q_start + new_pos, by, d], v_scale[bz, by]
                            )
                else:
                    for i, d in T.Parallel(block_m, dim):
...
class _GQAPrefillPagedWithKVCacheRopeAppendKernel(Kernel):
    ...
    def forward(
        ...
        kernel = _gqa_prefill_paged_with_kv_cache_rope_append_kernel(
            ...
            self.rope_layout,
            self.dtype_str,
            self.dtype_to_str(self.cache_dtype),
            self.dtype_to_str(self.rope_dtype),
        )
...
class _GQAPagedAttentionKernel(PagedPrefillKernel):
    ...
    def _build_program(self) -> None:
        ...
        if self.fuse_rope:
            if self.rotary_dim is None or self.max_position is None:
                raise ValueError("fused-RoPE paged prefill requires max_position and rotary_dim")
            input_dtype = fp8_dtype() if self.native_fp8 else self.dtype
            cache_dtype = fp8_dtype() if self.fp8_cache else self.dtype
    ...
    def forward(
        ...
        if self._append is not None and self.append_kv:
            self._append(
                k_new,
                v_new,
                k_pages,
...
class GQAPrefillPagedWithFP8KVCacheFwdKernel(_GQAPagedAttentionKernel):
    ...
    @classmethod
    def applies(cls, call) -> bool:
        return not call.is_fp8 and call.cache_dtype == fp8_dtype()
...
class GQAPrefillPagedNativeFP8TensorCoreFwdKernel(_GQAPagedAttentionKernel):
    ...
    @classmethod
    def applies(cls, call) -> bool:
        return call.is_fp8 and call.cache_dtype == fp8_dtype() and call.dim == 128
...
class GQAPrefillPagedWithKVCacheRopeFwdKernel(_GQAPagedAttentionKernel):
    ...
    def applies(cls, call) -> bool:
        return (
            not call.is_fp8
            and bool(call.fuse_rope)
            and call.rope_layout in ("neox", "interleaved")
            and call.cache_dtype == call.dtype
        )
