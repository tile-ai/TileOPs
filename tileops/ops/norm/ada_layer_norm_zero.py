...
class AdaLayerNormZeroOp(Op):
    ...
    The scale, shift, and gate tensors are per-token (shape matching x), not
    per-feature. Linear projection from a conditioning input to scale/shift/gate
    ...
    Args:
        ...
        tune: If True, run autotuning.

    Example:
        >>> op = AdaLayerNormZeroOp(M=1024, N=4096, dtype=torch.float16)
        >>> x = torch.randn(1024, 4096, dtype=torch.float16, device="cuda")
        >>> scale = torch.ones(1024, 4096, dtype=torch.float16, device="cuda")
        >>> shift = torch.zeros(1024, 4096, dtype=torch.float16, device="cuda")
        >>> gate = torch.ones(1024, 4096, dtype=torch.float16, device="cuda")
    ...
    def forward(
        ...
        """Compute AdaLN-Zero: y = gate * (scale * LayerNorm(x) + shift).

        Args:
            x: Input tensor with last dim == N.
            scale: Per-token scale tensor, same shape as x.
            shift: Per-token shift tensor, same shape as x.
            gate: Per-token gate tensor, same shape as x.
        ...
        if scale.shape != x.shape:
            raise ValueError(
                f"Expected scale shape {x.shape}, got {scale.shape}"
            )
        if shift.shape != x.shape:
            raise ValueError(
                f"Expected shift shape {x.shape}, got {shift.shape}"
            )
        if gate.shape != x.shape:
            raise ValueError(
                f"Expected gate shape {x.shape}, got {gate.shape}"
            )
