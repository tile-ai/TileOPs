...
class AdaLayerNormOp(Op):
    ...
    The scale and shift tensors are per-token (shape matching x), not per-feature.
    Linear projection from a conditioning input to scale/shift is the caller's
    responsibility.
    ...
    Example:
        >>> op = AdaLayerNormOp(M=1024, N=4096, dtype=torch.float16)
    ...
    def forward(
        ...
        """Compute AdaLN: y = scale * LayerNorm(x) + shift.
