"""Legacy manifest-layer roofline evaluator.

Scheduled for removal once per-op codegen emits each Op's own
``eval_roofline()``. The target design routes roofline expression
evaluation through codegen-emitted plain Python per the roofline spec
(``docs/roofline.md`` §4.4.6); the evaluator surface exposed here is
part of the transitional state.

Do not add new callers. Existing callers in ``tileops.manifest``,
benchmark harnesses and tests continue to import the public symbols
(``eval_roofline``, ``resolve_roofline_vars``, ``has_roofline_vars``)
via re-export from ``tileops.manifest``.
"""
...
from . import manifest as _manifest_core

# ---------------------------------------------------------------------------
# Safe arithmetic evaluator (AST-based) — used for inline flops/bytes.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# eval_roofline — dispatches to inline arithmetic or a human-authored func.
# ---------------------------------------------------------------------------
...
def eval_roofline(op_name: str, **variables: float) -> tuple[float, float]:
    ...
    """
    ops = _manifest_core._load_manifest()
...
# ---------------------------------------------------------------------------
# roofline.vars resolver — shape-derived variable resolution.
# ---------------------------------------------------------------------------
