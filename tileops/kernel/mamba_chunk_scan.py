...
class MambaChunkScanKernel(nn.Module):
    ...
    def __init__(self,
        ...
        self.fwd_program = _chunk_scan_fwd(batch, seq_len, chunk_size, groups, heads, dim,
                                           dstate)(**self.config)
    ...
    def profile(self, warmup=500):
        if self.tune_config is None and self.tune:
            self.autotune()
        if self.tune_config:
            self.fwd_program = _chunk_scan_fwd(self.batch, self.seq_len, self.chunk_size,
