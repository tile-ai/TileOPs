...
class MoePermuteNopadKernel(Kernel):
    ...
    Output perm_h has exactly T*K rows — no conservative upper-bound allocation.
    ...
    def forward(
        ...
        Returns:
            perm_h:                    [T*K, H] tight hidden states (no padding)
            true_offsets:              [E] int32 tight start per expert
            true_sizes:                [E] int32 true token count per expert
            expert_first_token_offset: [E+1] int64 non-padded exclusive prefix-sum
        ...
        true_offsets = torch.empty(self.num_experts, dtype=torch.int32, device=dev)
        true_sizes = torch.empty(self.num_experts, dtype=torch.int32, device=dev)
