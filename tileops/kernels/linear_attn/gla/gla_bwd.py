...
# ---------------------------------------------------------------------------
# Pass 2a: intra-chunk gradients (sub-chunk GEMM tiled dq/dk + dv_intra)
# ---------------------------------------------------------------------------

def _gla_bwd_intra_kernel(
    batch: int,
    seq_len: int,
    heads: int,
    dim_k: int,
    ...
    """Compute intra-chunk dq, dk, dv per chunk independently.

    Uses sub-chunk GEMM tiling (following FLA) for dq/dk:
      dq[i,k] = sum_{j<=i} dA[i,j] * k[j,k] * exp(g[i,k] - g[j,k])
    is factored as:
      Off-diagonal (s_j < s_i): GEMM with exp normalization
        k_shifted = k * exp(g_ref - g_cumsum)
        dq += dA_sub @ k_shifted  (GEMM)
        dq *= exp(g_cumsum - g_ref)  (restore)
      Diagonal (s_j == s_i): element-wise loop over BC positions

    Reference:
        FLA chunk_gla_bwd_kernel_intra
    """
    accum_dtype = "float32"
    num_chunks = seq_len // chunk_size
    BT = chunk_size
    ...
    def _intra_func(num_stages, threads=128):
        q_shape = [batch, seq_len, heads, dim_k]
        ...
        def _main(
            ...
            with T.Kernel(batch * heads * num_chunks, threads=threads) as bx:
                ...
                # ---- A[i,j] = sum_k q*k*exp(g_i - g_j), causal ----
                ...
                # ============================================================
                # Sub-chunk tiled dq (FLA-style)
                # dq[i,k] = sum_{j<=i} dA[i,j] * k[j,k] * exp(g[i]-g[j])
                # ============================================================
                ...
                for s_i in T.Serial(NS):
                    ...
                    # Off-diagonal: s_j < s_i → GEMM
                    # g_ref = g_cumsum[s_i * BC] (first pos of target)
                    for s_j in T.Serial(NS):
                        if s_j < s_i:
                            for i_t, i_j in T.Parallel(BC, BC):
                                dA_sub[i_t, i_j] = A_s[
                                    s_i * BC + i_t, s_j * BC + i_j]
                            # k_shifted = k * exp(g_ref - g_cumsum[j])
                            # g_ref more negative than g[j] → exponent ≤ 0
                    ...
                    # Restore: dq *= exp(g_cumsum[i] - g_ref)
                    ...
                    # Diagonal: s_j == s_i, element-wise over BC
                    # Read dA diagonal and k diagonal into fragments
                    # to avoid the T.if_then_else + bf16 smem bug.
                    for j_local in T.Serial(BC):
                        # Load dA column and k row for this j
                    ...
                    # Write dq for this sub-chunk
                    for i_local, i_k in T.Parallel(BC, dim_k):
                        dq_out[i_b, chunk_start + s_i * BC + i_local,
                               i_h, i_k] = dq_sub[i_local, i_k]

                # ============================================================
                # Sub-chunk tiled dk (FLA-style)
                # dk[j,k] = sum_{i>=j} dA[i,j] * q[i,k] * exp(g[i]-g[j])
                # ============================================================
                ...
                for s_j in T.Serial(NS):
                    ...
                    # Off-diagonal: s_i > s_j → GEMM
                    # g_ref = g_cumsum[(s_j+1)*BC - 1] (last pos of target)
                    for s_i in T.Serial(NS):
                        if s_i > s_j:
                            for i_i, i_j in T.Parallel(BC, BC):
                                dA_sub[i_i, i_j] = A_s[
                                    s_i * BC + i_i, s_j * BC + i_j]
                            # q_shifted = q * exp(g_cumsum[i] - g_ref)
                            # g[i] more negative than g_ref → exponent ≤ 0
                            for i_t, i_k in T.Parallel(BC, dim_k):
                    ...
                    # Restore: dk *= exp(g_ref - g_cumsum[j])
                    ...
                    # Diagonal: s_i == s_j, element-wise over BC
                    ...
                    # Write dk for this sub-chunk
                    for j_local, i_k in T.Parallel(BC, dim_k):
                        dk_out[i_b, chunk_start + s_j * BC + j_local,
                               i_h, i_k] = dk_sub[j_local, i_k]
...
# ---------------------------------------------------------------------------
# Pass 2b: inter-chunk gradients (GEMM-based dv/dq/dk inter + dg)
# (kept for reference / fallback)
# ---------------------------------------------------------------------------

def _gla_bwd_inter_kernel(
    batch: int,
    seq_len: int,
    heads: int,
    dim_k: int,
    ...
    """Compute inter-chunk dv, dq, dk contributions and dg.

    Reads dq_intra/dk_intra from intra kernel output.
    Combines intra + inter for final dq, dk, dv, and computes dg.
    """
    accum_dtype = "float32"
    num_chunks = seq_len // chunk_size
    ...
    def _inter_func(num_stages, threads=128):
        q_shape = [batch, seq_len, heads, dim_k]
        k_shape = [batch, seq_len, heads, dim_k]
        v_shape = [batch, seq_len, heads, dim_v]
        g_cumsum_shape = [batch, seq_len, heads, dim_k]
        ...
        h_shape = [batch, num_chunks + 1, heads, dim_k, dim_v]
        dh_shape = [batch, num_chunks, heads, dim_k, dim_v]
        dq_intra_shape = [batch, seq_len, heads, dim_k]
        dk_intra_shape = [batch, seq_len, heads, dim_k]
        dv_intra_shape = [batch, seq_len, heads, dim_v]
        dq_shape = [batch, seq_len, heads, dim_k]
        dk_shape = [batch, seq_len, heads, dim_k]
        dv_shape = [batch, seq_len, heads, dim_v]
        dg_shape = [batch, seq_len, heads, dim_k]
        ...
        def _main(
            ...
            h: T.Tensor(h_shape, accum_dtype),
            dh: T.Tensor(dh_shape, accum_dtype),
            dq_intra: T.Tensor(dq_intra_shape, accum_dtype),
            dk_intra: T.Tensor(dk_intra_shape, accum_dtype),
            dv_intra: T.Tensor(dv_intra_shape, accum_dtype),
            dq_out: T.Tensor(dq_shape, accum_dtype),
            dk_out: T.Tensor(dk_shape, accum_dtype),
            dv_out: T.Tensor(dv_shape, accum_dtype),
            dg_out: T.Tensor(dg_shape, accum_dtype),
            ...
            with T.Kernel(batch * heads * num_chunks, threads=threads) as bx:
                ...
                do_s = T.alloc_shared([chunk_size, dim_v], dtype)
                g_cumsum_s = T.alloc_shared([chunk_size, dim_k], accum_dtype)
                k_gated_s = T.alloc_shared([chunk_size, dim_k], dtype)
                h_cast_s = T.alloc_shared([dim_k, dim_v], dtype)
                dh_cast_s = T.alloc_shared([dim_k, dim_v], dtype)
                ...
                T.copy(v[i_b, chunk_start:chunk_start + chunk_size, i_h, :],
                       v_s, disable_tma=True)
                T.copy(do[i_b, chunk_start:chunk_start + chunk_size, i_h, :],
                       do_s, disable_tma=True)
                ...
                dv_frag = T.alloc_fragment([chunk_size, dim_v], accum_dtype)
                T.fill(dv_frag, 0.0)
                T.gemm(k_gated_s, dh_cast_s, dv_frag,
                       policy=T.GemmWarpPolicy.FullRow)
                ...
                for i_t, i_v in T.Parallel(chunk_size, dim_v):
                    dv_out[i_b, chunk_start + i_t, i_h, i_v] = (
                        dv_intra[i_b, chunk_start + i_t, i_h, i_v]
                        + dv_frag[i_t, i_v])

                # ==== dq_inter = do @ h^T ====
                dq_inter_frag = T.alloc_fragment([chunk_size, dim_k], accum_dtype)
                T.fill(dq_inter_frag, 0.0)
                T.gemm(do_s, h_cast_s, dq_inter_frag, transpose_B=True,
                       policy=T.GemmWarpPolicy.FullRow)
                ...
                for i_t, i_k in T.Parallel(chunk_size, dim_k):
                    dq_out[i_b, chunk_start + i_t, i_h, i_k] = (
                        dq_intra[i_b, chunk_start + i_t, i_h, i_k]
                        + scale * dq_inter_frag[i_t, i_k]
                        * T.exp2(g_cumsum_s[i_t, i_k] * LOG2_E))

                # ==== dk_inter = v @ dh^T ====
                dk_inter_frag = T.alloc_fragment([chunk_size, dim_k], accum_dtype)
                T.fill(dk_inter_frag, 0.0)
                T.gemm(v_s, dh_cast_s, dk_inter_frag, transpose_B=True,
                       policy=T.GemmWarpPolicy.FullRow)
                ...
                for i_t, i_k in T.Parallel(chunk_size, dim_k):
                    dk_out[i_b, chunk_start + i_t, i_h, i_k] = (
                        dk_intra[i_b, chunk_start + i_t, i_h, i_k]
                        + dk_inter_frag[i_t, i_k]
                        * T.exp2((g_last[i_k] - g_cumsum_s[i_t, i_k]) * LOG2_E))

                # ==== dg ====
                # dg_inter = sum_v(h * dh) * exp(g_last) + sum_t(k * dk_inter_gated)
                dg_inter = T.alloc_shared([dim_k], accum_dtype)
                for i_k in T.Parallel(dim_k):
                    dg_inter[i_k] = 0.0
                for i_v2 in T.Serial(dim_v):
                ...
                # Correction: k * dk_inter_gated
                corr_s = T.alloc_shared([chunk_size, dim_k], accum_dtype)
                for i_t, i_k in T.Parallel(chunk_size, dim_k):
                    corr_s[i_t, i_k] = (
                        T.cast(k_s[i_t, i_k], accum_dtype)
                        * dk_inter_frag[i_t, i_k]
                        * T.exp2((g_last[i_k] - g_cumsum_s[i_t, i_k]) * LOG2_E))
                for i_t in T.Serial(chunk_size):
                    for i_k in T.Parallel(dim_k):
                        dg_inter[i_k] = dg_inter[i_k] + corr_s[i_t, i_k]

                # dg_local = q * dq_total - k * dk_total
                # Read dq_intra/dk_intra from global, combine with inter
                for i_t, i_k in T.Parallel(chunk_size, dim_k):
                    g_cumsum_s[i_t, i_k] = (
                        T.cast(q_s[i_t, i_k], accum_dtype)
                        * (dq_intra[i_b, chunk_start + i_t, i_h, i_k]
                ...
                # Reverse cumsum
                for s in T.Serial(chunk_size - 1):
                    i_t_rev = chunk_size - 2 - s
                    for i_k in T.Parallel(dim_k):
                        g_cumsum_s[i_t_rev, i_k] = (
...
class GLABwdKernel(Kernel):
    """GLA backward kernel — three-pass architecture.

    Pass 1 (sequential reverse, B*H blocks): Accumulate dh per chunk.
    Pass 2a (parallel, B*H*NC blocks): Intra-chunk dq/dk/dv (element-wise).
    Pass 2b (parallel, B*H*NC blocks): Inter-chunk dq/dk/dv (GEMM) + dg.

    Splitting intra/inter reduces shared memory per kernel → higher occupancy.
