...
# =============================================================================
# Legacy kernel2 (kept for reference / backward compat)
# =============================================================================
            ...
            with T.Kernel(batch, head, threads=threads) as (bid, hid):
                ...
                for t in T.Pipelined(num_chunks, num_stages=num_stages):
                    ...
                    for i, j in T.Parallel(block_C, dim_v):
                        v_new_c[i, j] = u_shared[i, j] - ws_frag[i, j] * T.exp2((g_c[i] + g_last_val) * _LOG2E)
    ...
    def autotune(self, warmup: int = 10, rep: int = 10) -> None:
        ...
        for bv in bv_candidates:
            ...
            if tuned_h.config is not None:
                # Access latency from the tuner result
