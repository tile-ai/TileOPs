...
# =============================================================================
# Legacy monolithic kernel2_bwd (kept for reference / fallback)
# =============================================================================
        ...
        def _kernel2_bwd_body(
            ...
            with T.Kernel(batch, head, threads=threads) as (bid, hid):
                ...
                for t in T.Pipelined(num_chunks, num_stages=num_stages):
                    ...
                    # dh_frag starts at 0 (contributions added in steps below)
                    T.clear(dh_frag)
                    ...
                    # Step 4: apply Γ weighting to d_attn for dq/dk + dg from Γ
                    for i, j in T.Parallel(block_C, block_C):
                    ...
                    # Step 5: P = w_c*exp(g+g_last), dP = -d_v_new_c @ h_c^T, dh_frag -= P^T @ d_v_new_c
                    for i, j in T.Parallel(block_C, dim_k):
                    ...
                    T.reduce_sum(d_g_last_partial2, d_g_last_scalar2, dim=0)
...
def _gated_deltanet_bwd_impl(
    ...
    dk = dk + dk_wu
    # Reverse cumsum: dg w.r.t. g_cum → dg w.r.t. raw g
    dg = dg_cum.float().reshape(B, H, S_len // BC, BC).flip(-1).cumsum(-1).flip(-1).reshape(B, H, S_len).to(g.dtype)
