...
@triton.autotune(
    configs=[
        ...
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 32, 'NUM_SM': 128}),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 32, 'NUM_SM': 128}),
    ...
    for g in range(group_size):
        ...
        while (tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles):
            ...
            for k in range(0, tl.cdiv(gk, BLOCK_SIZE_K)):
                a = tl.load(a_ptr + rm[:, None] * lda + rk[None, :], 
                           mask=(rm[:, None] < gm) & (rk[None, :] < gk), other=0.0)
                b = tl.load(b_ptr + rk[:, None] * ldb + rn[None, :], 
                           mask=(rk[:, None] < gk) & (rn[None, :] < gn), other=0.0)
                acc += tl.dot(a, b)
                rk += BLOCK_SIZE_K
            
            tl.store(c_ptr + rm[:, None] * ldc + rn[None, :], 
                    acc.to(tl.float16), 
    ...
    for g in range(group_size):
        ...
        while (tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles):
            ...
            for n in range(0, tl.cdiv(gn, BLOCK_SIZE_K)):
                ...
                a = tl.load(a_ptr + rm[:, None] * lda + rn[None, :], mask=(rm[:, None] < gm) & (rn[None, :] < gn), other=0.0)
                b = tl.load(b_ptr + rn[:, None] * ldb + rk[None, :], mask=(rn[:, None] < gn) & (rk[None, :] < gk), other=0.0)
                acc += tl.dot(a, b)
            
            tl.store(c_ptr + rm[:, None] * ldc + rk[None, :], acc.to(tl.float16), mask=(rm[:, None] < gm) & (rk[None, :] < gk))
            tile_idx += NUM_SM
    ...
    for g in range(group_size):
        ...
        while (tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles):
            ...
            for m_block in range(0, tl.cdiv(gm, BLOCK_SIZE_K)):
                ...
                a = tl.load(a_ptr + rk[:, None] * lda + rm[None, :], mask=(rk[:, None] < gk) & (rm[None, :] < gm), other=0.0)
                b = tl.load(b_ptr + rm[:, None] * ldb + rn[None, :], mask=(rm[:, None] < gm) & (rn[None, :] < gn), other=0.0)
                acc += tl.dot(a, b)
            
            tl.store(c_ptr + rk[:, None] * ldc + rn[None, :], 
                    acc.to(tl.float16), 
...
def grouped_gemm_tt_kernel(
    group_dO_ptrs, group_A_ptrs, group_C_ptrs,
    group_gemm_sizes, g_lds, group_size,
    NUM_SM: tl.constexpr,
    BLOCK_SIZE_M: tl.constexpr,
    BLOCK_SIZE_N: tl.constexpr,
...
def prepare_nt_inputs(batch_sum: int, batch_count: int, K: int, N: int, dtype=torch.float16):
    ...
    for i in range(batch_count_calc - 1):
        batch_padded_offsets_list.append(
            batch_padded_offsets_list[-1] + math.ceil((batch_sizes_list[i] + 1) / padding_M) * padding_M
        )
...
def prepare_grouped_tensors_nt(A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor):
    ...
    return (torch.tensor(A_addrs, device=A.device),
            torch.tensor(B_addrs, device=B.device),
            torch.tensor(C_addrs, device=A.device),
            torch.tensor(g_sizes, dtype=torch.int32, device=A.device),
            torch.tensor(g_lds, dtype=torch.int32, device=A.device),
            group_C)
...
def prepare_grouped_tensors_nn(A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor):
    ...
    for i, size in enumerate(batch_sizes_list):
        ...
        N = A.shape[1]
        K = B.shape[2]
        assert B.shape[1] == N, f"B.shape[1]={B.shape[1]} != N={N}, B should have shape (batch_count, N, K)"
...
def prepare_grouped_tensors_tt(dO: torch.Tensor, A: torch.Tensor, batch_sizes: torch.Tensor):
    ...
    return (
        torch.tensor(dO_addrs, device=dO.device),
        torch.tensor(A_addrs, device=A.device),
        torch.tensor(C_addrs, device=dO.device),
        torch.tensor(g_sizes, dtype=torch.int32, device=dO.device),
        torch.tensor(g_lds, dtype=torch.int32, device=dO.device),
        group_C
    )
...
def grouped_gemm_nt(A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor, 
    ...
    (d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, group_C) = prepare_grouped_tensors_nt(A, B, batch_sizes)
    
    grid = lambda META: (META['NUM_SM'],)
    grouped_gemm_nt_kernel[grid](
        d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, len(batch_sizes)
    )
...
def grouped_gemm_nn(A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor, 
                   batch_offsets: torch.Tensor, batch_padded_offsets: torch.Tensor) -> torch.Tensor:
    (d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, group_C) = prepare_grouped_tensors_nn(A, B, batch_sizes)
    
    grid = lambda META: (META['NUM_SM'],)
    grouped_gemm_nn_kernel[grid](
        d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, len(batch_sizes)
    )
...
def grouped_gemm_tn(A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor,
                   batch_offsets: torch.Tensor, batch_padded_offsets: torch.Tensor) -> torch.Tensor:
    (d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, group_C) = prepare_grouped_tensors_tn(A, B, batch_sizes)
    
    grid = lambda META: (META['NUM_SM'],)
    grouped_gemm_tn_kernel[grid](
        d_a_ptrs, d_b_ptrs, d_c_ptrs, d_g_sizes, d_g_lds, len(batch_sizes)
    )
...
def grouped_gemm_tt(dO: torch.Tensor, A: torch.Tensor, batch_sizes: torch.Tensor, 
                   batch_offsets: torch.Tensor, batch_padded_offsets: torch.Tensor) -> torch.Tensor:
    (dO_addrs, A_addrs, C_addrs, g_sizes, g_lds, group_C) = prepare_grouped_tensors_tt(dO, A, batch_sizes)
    
    grid = lambda META: (META['NUM_SM'],)
    grouped_gemm_tt_kernel[grid](
        dO_addrs, A_addrs, C_addrs, g_sizes, g_lds, len(batch_sizes)
    )
...
def ref_program_nn(A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor,
    ...
    assert B.shape[0] == len(batch_sizes), f"B.shape[0]={B.shape[0]} != len(batch_sizes)={len(batch_sizes)}"
...
def ref_program_tt(dO: torch.Tensor, A: torch.Tensor, batch_sizes: torch.Tensor,
                  batch_offsets: torch.Tensor, batch_padded_offsets: torch.Tensor):
    output = torch.zeros((len(batch_sizes), dO.shape[1], A.shape[1]), device=dO.device, dtype=dO.dtype)
    start = 0
    for i, size in enumerate(batch_sizes):
        size_val = size.item()
        end = start + size_val
