import argparse
import torch
from tla import MAMBA_CHUNK_STATE_kernel
import math
...
def main():
    ...
    B = torch.empty((BATCH, SEQ_LEN, GROUPS, DSTATE),
    ...
    if args.tune:
        mamba_chunk_scan = MAMBA_CHUNK_STATE_kernel(BATCH,
                                                   HEADS,
                                                   GROUPS,
                                                   SEQ_LEN,
                                                   CHUNK_SIZE,
    ...
    o = mamba_chunk_scan(B, x, dt, dA_cumsum)
    print(o)
    latency = mamba_chunk_scan.profile()
    print(f"Latency: {latency:.4f} ms")
    mamba_chunk_scan.check(B, x, dt, dA_cumsum)
