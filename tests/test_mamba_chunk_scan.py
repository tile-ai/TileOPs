...
def main():
    ...
    cb = torch.empty((BATCH, NCHUNKS, GROUPS, CHUNK_SIZE, CHUNK_SIZE), dtype=torch.half).cuda().normal_(-1.0, 1.0)
    x = torch.empty((BATCH, SEQ_LEN, HEADS, DIM), dtype=torch.half).cuda().normal_(-1.0, 1.0)
    dt = torch.empty((BATCH, HEADS, NCHUNKS, CHUNK_SIZE), dtype=torch.half).cuda().normal_(-1.0, 1.0)
    dA_cumsum = torch.empty((BATCH, HEADS, NCHUNKS, CHUNK_SIZE), dtype=torch.half).cuda().normal_(-1.0, 1.0)
    C = torch.empty((BATCH, SEQ_LEN, GROUPS, DSTATE), dtype=torch.half).cuda().normal_(-1.0, 1.0)
    prev_states = torch.empty((BATCH, NCHUNKS, HEADS, DIM, DSTATE), dtype=torch.half).cuda().normal_(-1.0, 1.0)
    D = torch.empty((HEADS,), dtype=torch.half).cuda().normal_(-1.0, 1.0) 

    tune = False
    if tune:
        mamba_chunk_scan = MAMBA_CHUNK_SCAN_kernel(BATCH, HEADS, GROUPS, SEQ_LEN, CHUNK_SIZE, DIM, DSTATE, tune=True)
        ...
        mamba_chunk_scan = MAMBA_CHUNK_SCAN_kernel(BATCH, HEADS, GROUPS, SEQ_LEN, CHUNK_SIZE, DIM, DSTATE,
                                                   block_M=block_M, block_N=block_N, block_K=block_K, block_Dstate=block_Dstate, num_stages=num_stages, threads=threads)
        o = mamba_chunk_scan(cb, x, dt, dA_cumsum, C, prev_states, D)
        print(o)
        latency = mamba_chunk_scan.profile()
        print(f"Latency: {latency:.4f} ms")
