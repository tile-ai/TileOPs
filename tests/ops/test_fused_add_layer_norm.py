...
class FusedAddLayerNormTest(TestBase):
    ...
    def ref_program(
        ...
        # fused_add_layer_norm: y = LayerNorm(x + residual), also return x + residual
...
def _get_tolerances(dtype: torch.dtype) -> tuple[float, float]:
    if dtype == torch.float32:
        ...
        # Fused add increases intermediate precision diff vs pure-fp32 reference
        return 1e-2, 1e-2
    else:  # bfloat16
...
def test_fused_add_layer_norm_non_contiguous(m: int, n: int, dtype: torch.dtype) -> None:
    ...
    residual = residual_full[:, :n]  # non-contiguous slice
    ...
    y, pre_norm = op(x, residual, weight, bias)
    atol, rtol = _get_tolerances(dtype)
