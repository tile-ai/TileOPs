...
# ---------------------------------------------------------------------------
# Reduction subclasses without a scalar fast path must preserve the
# pre-existing ``ValueError("Input tensor must be at least 1D")`` for 0-D
# inputs (never the ``KeyError`` that an unguarded ``_TORCH_SCALAR_REF``
# lookup would raise).
# ---------------------------------------------------------------------------


_UNCOVERED_OPS = [
    ("argmax", "tileops.ops.reduction.argmax", "ArgmaxFwdOp"),
    ("argmin", "tileops.ops.reduction.argmin", "ArgminFwdOp"),
...
@pytest.mark.parametrize(
    "op_label,module_path,class_name",
    _UNCOVERED_OPS,
    ids=[entry[0] for entry in _UNCOVERED_OPS],
)
def test_uncovered_reduce_subclass_scalar_input_raises(
    op_label, module_path, class_name
) -> None:
    import importlib
    ...
    x = torch.tensor(1.0, dtype=torch.float32, device="cuda")
    op = op_cls(dtype=torch.float32, dim=0)
    with pytest.raises(ValueError, match="Input tensor must be at least 1D"):
        op(x)
