...
class FusedAddRmsNormTest(TestBase):
    ...
    def ref_program(
        ...
        # fused_add_rmsnorm: y = RmsNorm(x + residual), also return x + residual
        hidden = (x.float() + residual.float())
        rms = torch.sqrt(hidden.pow(2).mean(dim=-1, keepdim=True) + self.eps)
        y = ((hidden / rms) * weight.float()).to(x.dtype)
        return y, hidden.to(x.dtype)
...
def test_fused_add_rmsnorm_non_contiguous(m: int, n: int, dtype: torch.dtype) -> None:
    ...
    x = x_full[:, :n]
    residual_full = torch.randn(m, n * 2, dtype=dtype, device="cuda")
    residual = residual_full[:, :n]
    ...
    eps = 1e-6
    hidden = (x.contiguous().float() + residual.contiguous().float())
    rms = torch.sqrt(hidden.pow(2).mean(dim=-1, keepdim=True) + eps)
    y_ref = ((hidden / rms) * weight.float()).to(dtype)
    hidden_ref = hidden.to(dtype)

    y, pre_norm = op(x, residual, weight)
    atol, rtol = _get_tolerances(dtype)
    assert torch.allclose(y, y_ref, atol=atol, rtol=rtol), \
        f"Non-contiguous test failed (y), max err: {(y - y_ref).abs().max()}"
    assert torch.allclose(pre_norm, hidden_ref, atol=atol, rtol=rtol), \
        f"Non-contiguous test failed (pre_norm), max err: {(pre_norm - hidden_ref).abs().max()}"
...
def test_fused_add_rmsnorm_3d(batch: int, seq: int, hidden: int, dtype: torch.dtype) -> None:
    ...
    # Reference
    eps = 1e-6
    hidden_sum = (x.float() + residual.float())
    rms = torch.sqrt(hidden_sum.pow(2).mean(dim=-1, keepdim=True) + eps)
    y_ref = ((hidden_sum / rms) * weight.float()).to(dtype)
    hidden_ref = hidden_sum.to(dtype)

    y, pre_norm = op(x, residual, weight)
    atol, rtol = _get_tolerances(dtype)
    assert torch.allclose(y, y_ref, atol=atol, rtol=rtol), \
        f"3D test failed (y), max err: {(y - y_ref).abs().max()}"
    assert torch.allclose(pre_norm, hidden_ref, atol=atol, rtol=rtol), \
        f"3D test failed (pre_norm), max err: {(pre_norm - hidden_ref).abs().max()}"
