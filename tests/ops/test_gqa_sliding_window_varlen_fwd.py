    ...
    PARAMS = [
        ("batch, seqlens_q, seqlens_k, heads, heads_kv, dim,"
         " is_causal, wl, wr, dtype, tune", [
             ...
             (2, [256, 512], [256, 512], 8, 2, 64, True,  -1,  -1, torch.float16,  False),  # causal
             (2, [256, 512], [256, 512], 8, 2, 64, True, 128,  -1, torch.float16,  False),  # causal + wl
             (2, [256, 512], [256, 512], 8, 2, 64, False, -1,  -1, torch.float16,  False),  # bidirectional
             (2, [256, 512], [256, 512], 8, 2, 64, False, 64,  64, torch.float16,  False),  # window
             # ── KV-cache: seqlen_k > seqlen_q (offset > 0) ───────────────────
             (2, [64, 128],  [256, 512], 8, 2, 64, True,  -1,  -1, torch.float16,  False),  # causal kvcache
             (2, [64, 128],  [256, 512], 8, 2, 64, True, 128,  -1, torch.float16,  False),  # causal+wl kvcache
             (2, [64, 128],  [256, 512], 8, 2, 64, False, 64,  64, torch.float16,  False),  # window kvcache
             # ── bfloat16 ─────────────────────────────────────────────────────
             (2, [256, 512], [256, 512], 8, 2, 64, True,  -1,  -1, torch.bfloat16, False),  # causal bf16
             (2, [256, 512], [256, 512], 8, 2, 64, False, 64,  64, torch.bfloat16, False),  # window bf16
             # ── GQA ratios ───────────────────────────────────────────────────
             (2, [256, 512], [256, 512], 8, 8, 64, True,  -1,  -1, torch.float16,  False),  # MHA 1:1
             (2, [256, 512], [256, 512], 16, 1, 64, True, -1,  -1, torch.float16,  False),  # ratio 16:1
             # ── Mixed lengths within batch ────────────────────────────────────
             (3, [128, 256, 384], [128, 256, 384], 8, 2, 64, True, -1, -1, torch.float16, False),
             # ── Right window only ─────────────────────────────────────────────
             (2, [256, 512], [256, 512], 8, 2, 64, False, -1,  64, torch.float16,  False),
             # ── wl=0 boundary ────────────────────────────────────────────────
             (2, [128, 256], [128, 256], 8, 2, 64, True,   0,  -1, torch.float16,  False),
         ]),
    ]
