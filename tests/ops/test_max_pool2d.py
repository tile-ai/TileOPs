...
@pytest.mark.smoke
def test_max_pool2d_rejects_padding_pyTorch_rejects_when_dilated() -> None:
    with pytest.raises(ValueError, match="padding must be at most half"):
        MaxPool2dOp(
            n=1,
            c_in=8,
...
@pytest.mark.smoke
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_max_pool2d_return_indices_handles_all_negative_infinity(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("tileops.ops.op.get_sm_version", lambda: 80)
    x = torch.full((1, 3, 3, 1), float("-inf"), device="cuda", dtype=torch.float16)
    op = MaxPool2dOp(
        n=1,
        c_in=1,
        h_in=3,
    ...
    values, indices = op(x)
    expected_values = torch.full((1, 2, 2, 1), float("-inf"), device="cuda", dtype=torch.float16)
    expected_indices = torch.tensor([[[[0], [1]], [[3], [4]]]], device="cuda", dtype=torch.int64)
...
@pytest.mark.smoke
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_max_pool2d_returns_empty_tensor_for_zero_sized_output(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("tileops.ops.op.get_sm_version", lambda: 80)

    def fail_if_called(*args, **kwargs):
        raise AssertionError("kernel wrapper should not be called for zero-sized outputs")
    ...
    x = torch.randn(1, 1, 1, 1, device="cuda", dtype=torch.float16)
    out = op(x)
    assert out.shape == (1, 0, 0, 1)
    assert out.numel() == 0


@pytest.mark.smoke
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_max_pool2d_return_indices_returns_empty_tensors_for_zero_sized_output(
    ...
    x = torch.randn(1, 1, 1, 1, device="cuda", dtype=torch.float16)
    values, indices = op(x)
    assert values.shape == (1, 0, 0, 1)
    assert values.numel() == 0
    assert indices.shape == (1, 0, 0, 1)
    assert indices.dtype == torch.int64
