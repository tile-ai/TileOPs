...
class Fp8LightingIndexerOp(Op):
    ...
    def forward(self, index_q: torch.Tensor, index_k: torch.Tensor) -> torch.Tensor:
        index = index_q.to(torch.float8_e4m3fn)
        index_k, index_k_scale= per_custom_dims_cast_to_fp8(index_k , (0,) , False)

        return self.kernel(index_q, index_k, index_k_scale, self.weights, self.cu_seqlen_ks, self.cu_seqlen_ke)
