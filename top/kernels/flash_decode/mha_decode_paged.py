...
def main():
    ...
    real_seqlen_kv = torch.tensor([1024, 2048], dtype=torch.int32, device="cuda")
    ...
    kernel = mha_decode_paged_kernel(batch, heads, seqlen_q, seqlen_kv, dim, page_size, is_causal,
                                     dtype)
    output = kernel.forward(Q, K, V, real_seqlen_kv, block_table)
    print("kernel output shape:", output.shape)
    print("kernel output:", output)

    # torch reference: reassemble paged K/V to logical layout per batch, then SDPA
    for i_b in range(batch):
        q = Q[i_b:i_b + 1, :, :, :]  # [1, S_q, heads, dim]
        k_logical = torch.zeros(seqlen_kv, heads, dim, dtype=dtype, device="cuda")
        v_logical = torch.zeros(seqlen_kv, heads, dim, dtype=dtype, device="cuda")
        ...
        k_logical = k_logical[:real_seqlen_kv[i_b].item(), :, :]  # [real_len, heads, dim]
        v_logical = v_logical[:real_seqlen_kv[i_b].item(), :, :]

        k_logical = k_logical.unsqueeze(0)
        v_logical = v_logical.unsqueeze(0)
        q_bhsd = q.transpose(1, 2)  # [1, heads, S_q, dim]
        k_bhsd = k_logical.transpose(1, 2)  # [1, heads, S_kv, dim]
        v_bhsd = v_logical.transpose(1, 2)

        output_ref = F.scaled_dot_product_attention(q_bhsd, k_bhsd, v_bhsd)
        output_ref = output_ref.transpose(1, 2).contiguous()  # [1, S_q, heads, dim]
