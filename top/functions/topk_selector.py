...
class TopkSelectorCtx(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: FunctionCtx, index_scores: torch.Tensor, starts: torch.Tensor, ends: torch.Tensor,
                topk_selector_op: TopkSelectorOp) -> torch.Tensor:
...
class TopkSelectorFunc(Function):
    def __init__(self,
        ...
        self.topk_selector_op = TopkSelectorOp(
            batch,
            seq_len,
            topk,
            in_dtype,
    ...
    def forward(self, index_scores: torch.Tensor, starts: torch.Tensor, ends: torch.Tensor
                ) -> torch.Tensor:
