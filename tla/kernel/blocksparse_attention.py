...
@tilelang.jit
def blocksparse_flashattn_bwd(batch,
                              heads,
                              seq_len,
                              dim_qk,
                              dim_v,
...
class _blocksparse_attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, block_mask, block_M, block_N, causal, groups=1):
        ...
        mod = blocksparse_flashattn_fwd(BATCH, H, N_CTX, D_HEAD_QK, D_HEAD_V, causal, block_M,
                                        block_N, groups)
    ...
    def backward(ctx, do):
        ...
        delta = mod_prep(o, do)
        kernel = blocksparse_flashattn_bwd(BATCH, H, N_CTX, D_HEAD_QK, D_HEAD_V, ctx.causal,
...
class BlockSparseAttentionKernel(nn.Module):
    ...
    def profile(self, q, k, v, do, block_mask, warmup=100):
        ...
        with torch.no_grad():
            fwd_latency = do_bench(lambda: self.forward(q, k, v, block_mask), warmup=warmup)
            print(f"Fwd latency: {fwd_latency:.2f} ms")
            fwd_ref_latency = do_bench(lambda: self.ref_program(q, k, v, block_mask), warmup=warmup)
