...
@tilelang.jit
def blocksparse_flashattn_bwd(batch, heads, seq_len, dim_qk, dim_v, is_causal, block_M, block_N, groups=1):
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_N), batch, threads=128) as (bx, by, bz):
            K_shared = T.alloc_shared([block_N, dim_qk], dtype)
            ...
            for k in T.Pipelined(loop_st, loop_ed, num_stages=1):
                if block_mask[k] != 0:
                    ...
                    if is_causal:
                        for i, j in T.Parallel(block_N, block_M):
                            qkT[i, j] = T.if_then_else(by * block_N + i <= k * block_M + j, qkT[i, j],
                                                    0)
...
class _blocksparse_attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        delta = mod_prep(o, do)
        kernel = blocksparse_flashattn_bwd(BATCH, H, N_CTX, D_HEAD_QK, D_HEAD_V, ctx.causal, block_M, block_N,
                               groups)
...
class BlockSparseAttention_kernel(nn.Module):
    ...
    def backward(self, q, k, v, do, block_mask):
        o = self.attention(q, k, v, block_mask, self.block_M, self.block_N, self.causal,
                           self.groups)
    ...
    def check_forward(self, q, k, v, block_mask):
        o = self.attention(q, k, v, block_mask, self.block_M, self.block_N,
                           self.causal, self.groups)
    ...
    def check(self, q, k, v, do, block_mask):
    ...
    def profile(self, q, k, v, do, block_mask, warmup=100):
    
        with torch.no_grad():
            fwd_latency = do_bench(lambda: self.forward(q, k, v, block_mask),
                                   warmup=warmup)
            print(f"Fwd latency: {fwd_latency:.2f} ms")
            fwd_ref_latency = do_bench(lambda: self.ref_program(q, k, v, block_mask),
                                       warmup=warmup)
            print(f"Fwd ref latency: {fwd_ref_latency:.2f} ms")
            
        o = self.forward(q, k, v, block_mask)
