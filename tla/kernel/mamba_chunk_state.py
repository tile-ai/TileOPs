...
class _MAMBA_CHUNK_STATE_attention(torch.autograd.Function):
    ...
    def forward(ctx, B, x, dt, dA_cumsum, config):
        ...
        mod = _chunk_state_fwd(BATCH, SEQLEN, CHUNK_SIZE, NGROUPS, NHEADS, HEADDIM,
                               DSTATE)(**config)
...
class MAMBA_CHUNK_STATE_kernel(nn.Module):
    ...
    def __init__(self,
        ...
        self.fwd_program = _chunk_state_fwd(batch, seq_len, chunk_size, groups, heads, dim,
                                            dstate)(**self.config)
    ...
    def profile(self, warmup=500):
        if self.tune_config is None and self.tune:
            self.autotune()
        if self.tune_config:
            self.fwd_program = _chunk_state_fwd(self.batch, self.seq_len, self.chunk_size,
                                                self.groups, self.heads, self.dim,
