...
def get_configs():
    ...
    _configs = list(
        itertools.product(block_M, block_N, block_K, block_Dstate, num_stages))
...
def _chunk_scan_fwd(batch,
    ...
    def kernel_func(block_M, block_N, block_K, block_Dstate, num_stages,
        ...
        def _chunk_scan_fwd_main(cb: T.Tensor(
            ...
            with T.Kernel(nheads,
                ...
                cb_shared = T.alloc_shared((block_M, block_K),
                                           dtype,
                                           scope="shared.dyn")
                cb_local = T.alloc_fragment((block_M, block_K), dtype)
                dA_cs_k_shared = T.alloc_shared((block_K),
                                                dtype,
                                                scope="shared")
                ...
                x_shared = T.alloc_shared((block_K, block_N),
                                          dtype,
                                          scope="shared.dyn")
                dA_cs_m_shared = T.alloc_shared((block_M),
                                                dtype,
                                                scope="shared")
                scale_m_local = T.alloc_fragment((block_M), accum_dtype)
                C_shared = T.alloc_shared((block_M, block_Dstate), dtype)
                prev_state_shared = T.alloc_shared((block_N, block_Dstate),
                                                   dtype)
                D_local = T.alloc_fragment((1), accum_dtype)
                x_residual_shared = T.alloc_shared((block_M, block_N),
                                                   dtype,
                                                   scope="shared.dyn")
                x_residual_local = T.alloc_fragment((block_M, block_N),
                                                    accum_dtype)
                ...
                T.annotate_layout({
                    ...
                    cb_shared:
                    tilelang.layout.make_swizzled_layout(cb_shared),
                    x_residual_shared:
                    tilelang.layout.make_swizzled_layout(x_residual_shared)
                })
                ...
                T.copy(
                    C[batch_idx, chunk_idx * chunk_size +
                      m_idx * block_M:chunk_idx * chunk_size +
                      (m_idx + 1) * block_M, bz // (nheads // ngroups),
                      0:block_Dstate], C_shared)
                T.copy(
                    prev_states[batch_idx, chunk_idx, bz,
                ...
                for k in T.Pipelined(loop_range, num_stages=num_stages):
                    T.copy(
                        cb[batch_idx, chunk_idx, bz // (nheads // ngroups),
                           m_idx * block_M:(m_idx + 1) * block_M,
                           k * block_K:(k + 1) * block_K], cb_shared)
                    ...
                    T.copy(
                        x[batch_idx, chunk_idx * chunk_size +
                          ...
                          (k + 1) * block_K, bz,
                          n_idx * block_N:(n_idx + 1) * block_N], x_shared)
    ...
    if tune:
        ...
        def kernel(block_M=None,
                   ...
                   num_stages=None,
                   threads=None):
            return kernel_func(block_M, block_N, block_K, block_Dstate,
                               num_stages, threads)
        ...
        @tilelang.jit(out_idx=[7])
        def kernel(block_M, block_N, block_K, block_Dstate, num_stages,
                   threads):
            return kernel_func(block_M, block_N, block_K, block_Dstate,
                               num_stages, threads)
...
class _MAMBA_CHUNK_SCAN_attention(torch.autograd.Function):
    ...
    def forward(ctx, cb, x, dt, dA_cumsum, C, prev_states, D, config):
        ...
        DSTATE = C.shape[-1]
        mod = _chunk_scan_fwd(BATCH, SEQLEN, CHUNK_SIZE, NGROUPS, NHEADS,
...
class MAMBA_CHUNK_SCAN_kernel(nn.Module):
    ...
    def __init__(self,
        ...
        self.fwd_program = _chunk_scan_fwd(batch, seq_len, chunk_size, groups,
    ...
    def forward(self, cb, x, dt, dA_cumsum, C, prev_states, D):
        if self.tune_config is None and self.tune:
            self.autotune()
        if self.tune_config:
            o = self.attention(cb, x, dt, dA_cumsum, C, prev_states, D,
                               self.tune_config)
            return o
        o = self.attention(cb, x, dt, dA_cumsum, C, prev_states, D,
                           self.config)
    ...
    def autotune(self):
        best_result = _chunk_scan_fwd(self.batch,
                                      self.seq_len,
                                      self.chunk_size,
                                      self.groups,
                                      self.heads,
    ...
    def profile(self, warmup=500):
        if self.tune_config is None and self.tune:
            self.autotune()
        if self.tune_config:
            self.fwd_program = _chunk_scan_fwd(self.batch, self.seq_len,
    ...
    def ref_program(self, cb, x, dt, dA_cumsum, C, prev_states, D):
        ...
        dt_segment_sum = dA_cumsum[:, :, :, :, None] - dA_cumsum[:, :, :,
                                                                 None, :]
        ...
        state_decay_out = torch.exp(
            rearrange(dA_cumsum, "b h c l -> b c l h 1"))
