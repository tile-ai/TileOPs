...
1. **Optimize Architecture**: Eliminate unnecessary abstraction layers
1. **Improve Development Efficiency**: Faster development cycles and CI times
...
1. **Risk documentation**: Each module must clearly indicate experimental nature
1. **Independent test suite**: Tests in `tests/experimental/` run separately
1. **CI pipeline separation**: Experimental tests run independently without blocking main CI
1. **Promotion mechanism**: Mature experimental code can be promoted to `ops/`
...
class MLADecodeTest(TestBase):
    ...
    def gen_inputs(self) -> tuple:
        ...
        Q = torch.randn(
            self.batch, self.heads, self.dim, device=self.device, dtype=self.dtype
        )
        Q_pe = torch.randn(
            self.batch, self.heads, self.pe_dim, device=self.device, dtype=self.dtype
        )
        K = torch.randn(
            self.batch,
            self.seqlen_kv,
...
def test_mla_decode(
    ...
    """MLA Decode correctness test"""
    test = MLADecodeTest(
        batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, dtype, device
    )
...
class BenchmarkBase(ABC):
    ...
    def profile(self, backend: str = "tilelang") -> Dict[str, float]:
        ...
        return {"latency_ms": latency_ms, "tflops": tflops, "bandwidth_GB_s": bandwidth}
...
def test_mla_decode_benchmark(
    batch: int, heads: int, seqlen_kv: int, dim: int, dtype: torch.dtype
):
    """
    MLA Decode performance test.
    ...
    # Create profiler
    profiler = MLADecodeProfiler(op=mla_decode_with_kvcache, test_instance=test)
