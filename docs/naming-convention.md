...
1. **Non-abbreviation words use PascalCase.** `SlidingWindow`, `Decode`, `Paged`, `Varlen`. `MoE` is a literature-driven exception and stays mixed-case.
1. **Suffix is mandatory.** Op layer ends with `Op`; kernel layer ends with `Kernel`.
1. **No snake_case class names.** Anywhere.
1. **File names stay snake_case.** Only class names follow this convention.
...
deltanet_fwd.py    →  DeltaNetFwdOp / DeltaNetFwdKernel  (model name; see note)
fft_c2c.py         →  FFTC2COp / FFTC2CKernel  (C2C is in the vocabulary)
```

For each underscore-separated segment in the file name:

1. If the segment matches a vocabulary entry, write it ALL CAPS.
1. If the segment is the slug form of a proper noun or model name with an established literature spelling (e.g. `deltanet` → `DeltaNet`, `batchnorm` → `BatchNorm`, `layernorm` → `LayerNorm`), use that literature spelling.
1. Otherwise, capitalize the first letter (PascalCase).
...
| C2C    | Complex-to-Complex (FFT variant)                            | FFTW / cuFFT       |
...
If concatenation produces something genuinely unreadable, allow a non-abbreviation word as a visual separator (e.g. `FP8LinearOp` where `Linear` is not in the vocabulary). The boundary rule is: **inside the vocabulary → ALL CAPS; outside → PascalCase**. Do not use this escape hatch to PascalCase a vocabulary entry — `FP8GEMMOp` is correct, `FP8GemmOp` is not.
