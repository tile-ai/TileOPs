...
- **Rule.** Body sequence: (a) `self.<name> = <name>` per kwarg; (b) `self.dispatch_kernel(kernel_map)`; (c) initialise `self._kernel_cache: Dict[Hashable, Kernel] = {}`. **No kernel is constructed here**, for any op shape: the kernel is dtype-specialized and no dtype is known until `forward()` receives a tensor. What the ctor does resolve is the kernel *class* and the architecture check, both of which `dispatch_kernel` performs without a tensor.
  ...
  - The cache **value** is the kernel for ops where the incoming dtype is the whole story. When specialization implies more than a kernel — a compute dtype that differs from the semantic dtype (integer `reciprocal` computes in fp32), an output dtype the input does not give (`isnan` → bool), or bool operands reinterpreted as uint8 — the value is a frozen record carrying those alongside the kernel, so one lookup answers every dtype question the call needs. `tileops/ops/elementwise/_base.py::KernelEntry` is the reference. Do not spread the extra fields across separate `self.*` slots written as a side effect of building the kernel: a second dtype then leaves them describing the first.
