...
| Method                                             | Purpose                                                                                                                                                                                                                                                                    |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `get_or_build_kernel(name, inputs, *, key, build, params)` | Return the kernel for this call, building it once on a miss. `key` and `build` are the in-tree recipe; `inputs` describes an external target's builder. Optional `params` replaces unresolved manifest params for that external build and must be deterministic for the op instance and input signature because it is not part of the cache key |
