...
**R22. Mutated inputs.** A tensor input the op may write declares `mutated: true`. Across the operators the op registers, the inputs `mutates_args` names — matched to `signature.inputs` by the order an operator lists its tensor arguments — are exactly the ones marked. A mutated input stays an input: output arity does not change, and the return does not alias it. Contiguity normalization copies, so a mutated input it copied is written back into the caller's tensor after the launch.
