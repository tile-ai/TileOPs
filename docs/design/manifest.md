...
| 15  | `causal_topk_indices(B, S, H_kv, K, E, start, stride)` | positive `B, S, H_kv, K, E, stride`; `start >= 0`                                                | `[B, S, H_kv, K]`, row `t` draws distinct keys below `min(max(1, (t + start) // stride), E)`, padded with `E`                                                                     |
| 16  | `key_windows(L, first, count, side)`                   | non-empty positive list; `0 <= first`, `first + count <= sum(L)`; `side` is `'start'` or `'end'` | `[count]`, per packed position `p = first + i`: its segment's start, or `p + 1`                                                                                                   |
| 17  | `full(shape, value)`                                   | non-negative extents                                                                             | `shape`, every element `value`                                                                                                                                                    |
