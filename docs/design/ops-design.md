...
An op that serves several dtypes holds no scalar dtype state on an execution path. `self.dtype` exists for `eval_roofline` / `total_memory` and records the most recent call; reading it to decide anything about the call in flight is a bug, because the second dtype invalidates it.
