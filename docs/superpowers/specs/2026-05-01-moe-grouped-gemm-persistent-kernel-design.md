...
1. **Thread-0 串行扫描**：scheduler kernel 中 thread-0 顺序计算 prefix sum，O(E) 串行
1. **Dead CTA 浪费**：GEMM kernel grid = `max_tiles × N_tiles`，约 33% CTA 是 dead CTA（无效后立即 exit）
...
- 新增：`tile_counter` tensor（persistent across loop，每次 forward 前 fill\_(0)）
...
1. **Warp scan in TileLang**：`__shfl_up_sync` 可能需要 `T.call_extern` 调用
1. **SMEM layout 分区**：`s_cum[E+1]` 需要在整个 kernel 生命周期内保持有效（binary search 阶段仍需读取），不能被 A/B pipeline buffer 覆盖。具体布局：
...
1. **Backward 兼容**：新 kernel 作为独立类，不替换现有 `MoeGroupedGemmNopadKernel`，通过 Kernel 参数选择
