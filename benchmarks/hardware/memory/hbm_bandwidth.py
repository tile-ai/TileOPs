"""
Global Memory (HBM) Bandwidth Benchmark

Measures:
- Read / Write / Copy bandwidth across sizes and dtypes
- Strided read bandwidth (coalescing loss)

Peak reference: theoretical HBM bandwidth (e.g. 4800 GB/s for H200).
"""
...
def _measure_peak_copy_bw():
    """Measure peak copy bandwidth at 2GB as measured reference."""
    ...
    bw = calc_bandwidth_gbs(2 * nbytes, latency)  # read + write
    del src, dst
...
def benchmark_bandwidth():
    """Run full HBM bandwidth benchmark."""
    ...
    # Measure peak copy
    measured_bw = _measure_peak_copy_bw()
    set_measured_peak_bw(measured_bw)
    ...
    print(f"achieved_pct_of_peak is relative to theoretical ({theo_bw} GB/s)\n")

    # Use theoretical as the normalization denominator
    peak_bw = theo_bw or measured_bw

    # Read / Write / Copy across sizes and dtypes
