...
class AvgPool3dBenchmark(BenchmarkBase):
    ...
    def calculate_flops(self) -> Optional[float]:
        t = self.test
        out_d = (t.d_in + 2 * t.padding[0] - t.kernel_size[0] + t.stride[0] - 1) // t.stride[0] + 1 if t.ceil_mode else (
            (t.d_in + 2 * t.padding[0] - t.kernel_size[0]) // t.stride[0] + 1
        )
        out_h = (t.h_in + 2 * t.padding[1] - t.kernel_size[1] + t.stride[1] - 1) // t.stride[1] + 1 if t.ceil_mode else (
            (t.h_in + 2 * t.padding[1] - t.kernel_size[1]) // t.stride[1] + 1
        )
        out_w = (t.w_in + 2 * t.padding[2] - t.kernel_size[2] + t.stride[2] - 1) // t.stride[2] + 1 if t.ceil_mode else (
            (t.w_in + 2 * t.padding[2] - t.kernel_size[2]) // t.stride[2] + 1
        )
...
_AVG_POOL3D_BENCH_PARAMS = [
    ...
    pytest.param(1, 64, 8, 28, 28, (2, 3, 3), (2, 2, 2), (1, 1, 1), True, False, None, torch.float16, True, id="ceil-video"),
    pytest.param(1, 24, 10, 20, 22, (2, 2, 3), (2, 2, 2), (0, 1, 1), False, True, 7, torch.bfloat16, True, id="divisor-bf16"),
]
