...
def _torch_sdpa_dequant_fwd(case: GQAFp8TensorCoreBenchCase):
    ...
    Always available, so a row reports a comparison whether or not flash-attn 3
    is installed. Dequantizing the way ``tests/ops/attention/test_gqa_fp8.py``
    does keeps this the same computation the kernel performs, at the precision
    the op returns; ``scaled_dot_product_attention`` runs it without a
    ``seq_len x seq_len`` score matrix, which at the largest row would be 12 GiB.
    """
...
def test_gqa_prefill_fp8_tensor_core_bench(case: GQAFp8TensorCoreBenchCase) -> None:
    ...
    # fp8 quantization is the whole difference between the two, so the tolerance is
    # the one tests/ops/attention/test_gqa_fp8.py holds its dequantized reference to.
