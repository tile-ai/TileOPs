...
class grouped_gemm_nn_benchmark(Benchmark):
    ...
    def ref_program(self, A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor, batch_offsets: torch.Tensor, batch_padded_offsets: torch.Tensor):
        assert A.shape[0] == sum(batch_sizes)
        assert B.shape[0] == len(batch_sizes)
...
class grouped_gemm_benchmark(Benchmark):
    ...
    @property
    def total_flops(self):
        return (self.nt_benchmark.total_flops + self.nn_benchmark.total_flops + self.tn_benchmark.total_flops)
    
    @property
    def total_memory(self):
        return (self.nt_benchmark.total_memory + self.nn_benchmark.total_memory + self.tn_benchmark.total_memory)
    ...
    def ref_program(self, A: torch.Tensor, B: torch.Tensor, batch_sizes: torch.Tensor, batch_offsets: torch.Tensor, batch_padded_offsets: torch.Tensor):
        forward_output = self.nt_benchmark.ref_program(A, B, batch_sizes, batch_offsets, batch_padded_offsets)
        grad_output = torch.ones_like(forward_output)
        dA = self.nn_benchmark.ref_program(grad_output, B, batch_sizes, batch_offsets, batch_padded_offsets)
        # no need to transpose grad_output here, it is transposed in tn layout ref_program
        dB = self.tn_benchmark.ref_program(grad_output, A, batch_sizes, batch_offsets, batch_padded_offsets)
