...
1. Install commit-msg hook: `cp scripts/hooks/commit-msg .git/hooks/commit-msg && chmod +x .git/hooks/commit-msg`
...
- **Before every `git commit`**: run `pre-commit run --all-files` and fix all failures. Only stage the intended files after pre-commit passes. Never commit with `--no-verify`.
...
| `commit`                | Use when committing changes: validates commit message format against TileLang convention and ensures pre-commit passes before recording the commit |
