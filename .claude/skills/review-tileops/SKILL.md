...
- **Rule 2 — same-path 5-strike monitor** (`round-post.sh`): after each round, the loop extracts blocker paths from `round-NN.codex-blockers.json` and updates `review/region-history.json`, incrementing per-path counters for paths present this round and resetting counters for paths absent. The artifact is written by codex itself as the LAST step of its run (a JSON array of `{path, line, body}` for inline blockers it just submitted, or `[]` when the review is APPROVE/COMMENT) — the loop driver only reads it; there is no GitHub API fetch and therefore no pagination concern. A missing artifact is treated as `[]` (monitor-only fallback). When any path's counter reaches five consecutive blocker rounds, the loop ensures the PR carries the `agent-stuck` label (created idempotently in the repo if missing). The hook never mutates blockers, threads, comments, or the PR title — the label is the only externally-visible side effect. Missing `region-history.json` is treated as empty state, so this rule is backward compatible with older runs.
