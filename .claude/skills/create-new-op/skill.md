...
- **Do not create example files** when creating a new op

______________________________________________________________________

## Kernel Layer Requirements

The kernel (`tileops/kernels/<op_name>/`) **must** use TileLang primitives to implement GPU computation. Do **not** call PyTorch, NumPy, or other external libraries to perform the core computation inside `Kernel.forward`. Reference implementations: `tileops/kernels/gemm/gemm.py`, `tileops/kernels/gemv/gemv.py`, `tileops/kernels/flash_attn/fwd.py`.

The standard TileLang kernel pattern:

1. Define a `_<op_name>_kernel(params) -> Callable` function that captures static shapes/config
1. Inside it, decorate a function with `@tilelang.jit(out_idx=[...])` — its arguments become the tunable config
1. Inside the jit function, define `@T.macro` functions that contain `with T.Kernel()` blocks for GPU kernel launches
1. Inside the jit function, define the `@T.prim_func` that calls the macros — **`with T.Kernel()` blocks MUST be inside `@T.macro`, NOT directly in `@T.prim_func`**
1. Use TileLang primitives: `T.Kernel`, `T.Parallel`, `T.serial`, `T.Pipelined`, `T.alloc_shared`, `T.alloc_fragment`, `T.copy`, `T.gemm`, `T.clear`, `T.cast`, etc.
1. Register via `@torch.library.custom_op(...)` + `.register_fake(...)` for autograd/compile compatibility
1. In `Kernel.forward`, call the wrapped kernel function — never call `torch.fft`, `torch.matmul`, or equivalent directly

Reference: `tileops/kernels/flash_decode/gqa_decode.py` for proper `@T.macro` and `@T.prim_func` structure.
...
- [ ] `Kernel.forward` invokes the TileLang-compiled kernel — either `self.kernel(config...)(tensors)` or the `@torch.library.custom_op` wrapper (see `tileops/kernels/flash_decode/gqa_decode.py`) — never a PyTorch op (e.g. `torch.fft.fft`, `torch.matmul`) as the core computation
- [ ] Run `.claude/skills/check-kernel-format/skill.md` checklist in full before marking the op as complete
