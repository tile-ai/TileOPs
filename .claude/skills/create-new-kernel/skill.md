...
- **If a reference implementation is specified** (e.g. cuFFT, cuBLAS, PyTorch), ensure accumulation dtype and other hardcoded types match the reference unless higher precision is explicitly required. Mismatched precision (e.g. using `float64` accumulation for a `float32` reference) incurs severe hardware throughput penalties (30× on H200: 67 TFLOPS FP64 vs 1979 TFLOPS FP32). Use `accum_dtype = real_dtype` to derive accumulation dtype from the input dtype.
...
```python
# Replace KERNEL_NAME with the actual kernel name (e.g. _fft_c2c_lut_wrapped_kernel)
@_KERNEL_NAME_wrapped_kernel.register_fake
def _(
    param1: int,
    dtype: str,
    threads: int,
