...
`align-op` flips `status` and retargets `source.test` / `source.bench`; `align-family` writes no contractual field on any op ([manifest-spec.md](../../domain-rules/manifest-spec.md)).
