...
Invoke under `/loop` for auto-continue: `/loop /resolve-tileops-loop 1072`.
Invoke bare for a single round: `/resolve-tileops-loop 1072`.
...
if [ ! -f "$META" ]; then
  ...
  sed "s/__PR_NUMBER__/$PR/g" \
    "$REPO_PATH/.claude/skills/resolve-tileops-loop/policy-template.md" > "$RUN_DIR/policy.md"
  : > "$RUN_DIR/inbox.md"
...
- Under `/loop`: call `ScheduleWakeup` with `delaySeconds=POLL_INTERVAL_S`,
  `prompt=/resolve-tileops-loop <PR>`,
