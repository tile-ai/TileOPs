...
2. Read the external reference implementation and extract:
   - kernel stages
...
2. Plan implementation in strict order: `kernel -> op -> function -> layer`.
3. Define per-layer responsibilities and avoid cross-layer shortcuts.
4. List only level-required files/tests before coding (see checklist below).

### Phase C: Kernel Migration (TileLang)

1. Implement TileLang kernels by stage (same logical decomposition as reference).
2. Keep output semantics compatible with existing interfaces.
3. Handle core edge cases early (for example: empty paths / `num_topk == 0`).

### Phase D: Upper Layer Wiring

1. Add `op` wrapper for kernel invocation + runtime contract.
2. Add `function` API for reusable composition and shape/dtype validation.
3. Add `layer` abstraction only when module-style integration is needed.
4. Keep dependency direction one-way: `layer -> function -> op -> kernel`.
...
3. Run incremental tests first, then the relevant chain.

### Phase F: Cleanup and Finalization

1. Remove obsolete wrappers/helpers/classes.
2. Update exports/imports (`__init__.py`) to avoid stale symbols.
3. Re-run the main regression chain and summarize results.
