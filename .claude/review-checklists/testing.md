...
- [ ] **Test layer hygiene.** Tests must not import from `benchmarks/` per `docs/design/trust-model.md`. Cross-layer touches are reviewed against the cross-layer criteria in `pre-review.md` (oracle origin, coverage set, new-path coverage).
