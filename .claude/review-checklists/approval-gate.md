...
- [ ] **Reply discipline.** PR review replies stay terse — outcome only (`Done in <sha>.`, `Won't-fix: <one-line reason>.`). No commit-by-commit narration, "what I tried", thread/tally IDs, root-cause essays, or restatements of the design. Process detail ages out of context and clutters the thread. If a developer reply contains process narration, **REQUEST_CHANGES asking them to edit the comment down to a one-liner** before APPROVE.
