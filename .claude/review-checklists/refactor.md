...
- [ ] **Trust-layer scope.** Refactor stays within one trust layer per `docs/design/trust-model.md` (manifest / test (incl. `workloads/`) / implementation / benchmark). Cross-layer touches are reviewed against the cross-layer criteria in `pre-review.md` (oracle origin, coverage set, new-path coverage).
