...
- [ ] If feature is an op, spot-check the diff against at least one of `docs/design/ops-design.md` Slots S6 (class name ≡ manifest key), S12 (`__init__` kwargs derivable from manifest), S14 (`default_kernel_map` matches `source.kernel_map` verbatim), S16 (`forward` validation order), S18 (`_validate_dtypes` matches `dtype_combos`), S19 (`eval_roofline` plain-Python body); flag any divergence
