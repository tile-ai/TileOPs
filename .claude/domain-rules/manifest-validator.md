...
- Validator-related infrastructure: `scripts/validate_manifest.py`, the code under `src/tileops/manifest/`, `tests/test_validate_manifest.py`, `docs/design/manifest.md`. Only modify these when the PR is explicitly about manifest schema, layer-boundary policy, or validator infrastructure — and document the change in the PR body.
- If a new op cannot satisfy the current validator: stop and surface the mismatch. Do not "fix" it in the validator.
- Full spec: [docs/design/manifest.md](../../docs/design/manifest.md) (Validation section).
