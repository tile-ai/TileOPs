...
- A benchmark takes its op at construction and publishes every row under it; what distinguishes cases goes in `params`, never into the row's name. A comparison whose subject is not an op — a kernel strategy, a field of library implementations — decides something rather than tracking it: it asserts, or it lives in `benchmarks/studies/`, which the nightly sweep does not reach. `record()` takes an Op and nothing else. `scripts/check_bench_coverage.py` fails an op whose benchmark passed without recording it.
