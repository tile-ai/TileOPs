...
- Per-op workarounds (fallback / special branch / opt-out flag) MUST NOT be promoted to a base-class shared mechanism (mixin, class attribute, shared method) within the same op-family migration PR. Promotion requires a separate design PR justifying (a) the mechanism encodes a genuine family invariant — would belong in the base even if no op had taken a shortcut; (b) MUST / MAY scope is explicit, with MAY-with-default-flag (subclass negates the base) treated as the opt-out anti-pattern; (c) the mechanism does not let `status: spec-only` ops appear runnable. Pattern-level detection lives in the foundry articulation source-pair-2 detector.
