...
- `kernel_map` is the Op→Kernel dispatch registration table. Keys are snake_case dispatch identifiers, decoupled from class names (convention for new ops — some existing ops use PascalCase keys). Values are Kernel class names. The manifest declares this table; agents implement the listed Kernels. See [ops-design.md § Kernel Dispatch](../../docs/ops-design.md#kernel-dispatch-kernel_map).
