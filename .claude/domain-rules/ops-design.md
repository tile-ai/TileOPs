...
- `kernel_map` keys are stable snake_case dispatch identifiers (protocol-level). They MUST NOT be derived from `cls.__name__`. Renaming a Kernel class does not require renaming its dispatch key. For single-kernel ops prefer `<op_or_family>_kernel`; for multi-kernel ops use descriptive snake_case (e.g., `mha_bwd_preprocess_kernel`).
