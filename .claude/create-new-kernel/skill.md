...
Attention kernels have additional conventions beyond the general rules above.
...
- [ ] (Attention only) `is_causal: bool` is a parameter of the outermost kernel function
- [ ] (Attention only) File/class name includes variant suffix: `_fwd` / `_decode` / `_bwd`
- [ ] (Decode only) Both no-split and split-K `@T.prim_func` are implemented; `num_split` is in `default_config`
